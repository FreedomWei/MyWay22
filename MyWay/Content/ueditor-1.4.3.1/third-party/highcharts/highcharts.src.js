...
Pointer.prototype = {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		var chart = this.chart,
			...
			touc#h1Start = !singleTouch && pinchDown[1][sChartXY],
			touc#h1Now = !singleTouch && touches[1][sChartXY],
			outOfBounds,
			transformScale,
			scaleKey,
			setScale = function () {
				if (!singleTouch && mathAbs(touch0Start - touc#h1Start) > 20) { // Don't zoom if fingers are too close on this axis
					scale = mathAbs(touch0Now - touc#h1Now) / mathAbs(touch0Start - touc#h1Start);	
				}
		...
		if (outOfBounds) {
			...
			if (!singleTouch) {
				touc#h1Now -= 0.8 * (touc#h1Now - lastValidTouch[xy][1]);
			}
			...
			lastValidTouch[xy] = [touch0Now, touc#h1Now];
		}
